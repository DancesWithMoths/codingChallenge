...
def processData(filePaths):
    ...
    combinedDataSet = pd.concat(dataList, ignore_index=True)
...
if __name__ == '__main__':
    ...
    if optionSelect == '1':
        processData(['broker1.csv', 'broker2.csv'])
    elif optionSelect == '2':
        print(dataReporting(pd.read_csv('combinedDataSet.csv')))
    elif optionSelect == '3':
        brokerInfo = input("Input the ID or Name of Broker.")
        print(pd.read_csv('combinedDataSet.csv').query('brokerName == @brokerInfo or brokerId == @brokerInfo'))
    else:
